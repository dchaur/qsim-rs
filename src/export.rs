...
pub fn to_qasm(gates: &[Gate], num_qubits: usize) -> String {
    ...
    for gate in gates {
        let instruction = match gate.name {
            ...
            GateType::X => format!("x q[{}];", gate.targets[0]),
            GateType::Z => format!("z q[{}];", gate.targets[0]),
            GateType::CNOT => {
...
cx q[0],q[1];"#;
    ...
    #[test]
    fn export_to_qasm_all_gates() {
        let gates = vec![
            Gate::new(GateType::H, vec![0], None),
            Gate::new(GateType::X, vec![1], None),
            Gate::new(GateType::Z, vec![0], None),
            Gate::new(GateType::CNOT, vec![1], Some(vec![0])),
        ];
